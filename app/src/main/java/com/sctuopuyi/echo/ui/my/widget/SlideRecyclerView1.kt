...
class SlideRecyclerView1 : RecyclerView {
    ...
    private var mLastX: Float = 0.toFloat()   // 滑动过程中记录上次触碰点X
    private var mFirstX: Float = 0.toFloat()
    private var mFirstY: Float = 0.toFloat() // 首次触碰范围
    private var mIsSlide: Boolean = false   // 是否滑动子View
    private var mFlingView: ViewGroup? = null   // 触碰的子View
    private var mPosition: Int = 0  // 触碰的view的位置
    private var mMenuViewWidth: Int = 0    // 菜单按钮宽度
    ...
    override fun onInterceptTouchEvent(e: MotionEvent): Boolean {
        ...
        when (e.action) {
            MotionEvent.ACTION_DOWN -> {
                if (mScroller?.isFinished == false) {  // 如果动画还没停止，则立即终止动画
            ...
            MotionEvent.ACTION_MOVE -> {
                ...
                if (Math.abs(xVelocity) > SNAP_VELOCITY && Math.abs(xVelocity) > Math.abs(yVelocity) || Math.abs(
                        x - mFirstX
                    ) >= mTouchSlop && Math.abs(x - mFirstX) > Math.abs(y - mFirstY)
                ) {
                    mIsSlide = true
                    return true
                }
