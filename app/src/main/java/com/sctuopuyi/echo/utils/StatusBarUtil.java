...
/**
 * Created by fengmlo on 2018/1/25.
 */
...
public class StatusBarUtil {
     ...
     * 白状态栏、黑字
     */
    public static void WhiteStatusBarAndBlackFont(Activity activity) {
        StatusBarUtil.setStatusBarColor(activity, android.R.color.white);
     ...
     * 沉浸式加黑色字体
     */
    public static int setStatusTransAndBlackFont(Activity activity) {
        int result = 0;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                Window window = activity.getWindow();
