...
object FileUtils {
     ...
     * 获取obb的目录
     */
    fun getObbPath(context: Context): String {
     ...
     * 获取目录文件名
     */
    fun fileNameByExt(url: String?, ext: String): String? {
        return url?.substring(url.lastIndexOf("/") + 1, (url.length - ext.length) - 1)
    }
    ...
    fun getExternalName(url: String?): String? {
        val fileUrl = url?.reversed()
        return fileUrl?.substring(0, fileUrl.indexOf("."))?.reversed()
    }
     ...
     * 获取内部缓存目录
     */
    fun getCachDir(context: Context) = context.cacheDir.absolutePath


    /**
     * 获取内部文件目录
     */
    fun getFilesDir(context: Context) = context.filesDir.absolutePath
    ...
    fun treeFilesByMaxDepth(filePath: String?, startMaxDepth: Int? = 1): List<String> {
        if (filePath.isNullOrBlank())
            return arrayListOf()
        val file = File(filePath)
        val fileTree = file.walk()
        val files = fileTree.maxDepth(startMaxDepth ?: 1).asSequence().map {
            it.absolutePath
        }.filter {
            File(it).isFile
        }.toList()
        return if (files.isEmpty()) {
            treeFilesByMaxDepth(filePath, startMaxDepth?.plus(1))
        } else {
            files
        }
    }
     ...
     * 获取扩展文件目录
     */
    fun getExternalFilesDir(context: Context) = context.getExternalFilesDir(null).absolutePath


    /**
     * 获取视频文件的时长
     */
    fun getMediaFileDuration(videPath: String?): Int {
        val file = File(videPath)
