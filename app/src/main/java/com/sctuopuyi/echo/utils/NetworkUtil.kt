...
//            RxBus.publish(RxEvent.EventBean(RxEvent.EVENT_PAGE_TYPE_NETWORK_STATUS, result))
     ...
     * 判断gprs是否开启
     */
    fun isConnectGprs(context: Context?): Boolean {
        val obj =
            context?.getSystemService(Context.CONNECTIVITY_SERVICE) ?: return false
        val connMgr = obj as ConnectivityManager
        val dataNetworkInfo = connMgr.getNetworkInfo(ConnectivityManager.TYPE_MOBILE)
     ...
     * 设置gprs状态，开启和关闭gprs
     */
    fun setMobileDataEnabled(context: Context, flag: Boolean): Boolean {
        val obj =
            context?.getSystemService(Context.CONNECTIVITY_SERVICE) ?: return false
        val connMgr = obj as ConnectivityManager
        var setMobileDataEnabl: Method? = null
        try {
            setMobileDataEnabl = connMgr::class.java.getDeclaredMethod(
                "setMobileDataEnabled", Boolean::class.javaPrimitiveType
            )
            setMobileDataEnabl.invoke(connMgr, flag)
            return true
        } catch (e: Exception) {
            e.printStackTrace()
            return false
